...
$query="select user_id,agent_id,totalprice,agentcont_type,sendstatus,pid,is_QR,express_id from weixin_commonshop_orders where isvalid=true and batchcode='".$batchcode."'";
...
$sql="update weixin_commonshop_orders set sendstatus = 1,confirm_sendtime=now(),auto_receivetime = DATE_ADD( now(), INTERVAL ".$auto_cus_time." DAY ),expressnum='".$expressnum."',send_remarks='".$remarks."' where batchcode='".$batchcode."'";
mysql_query($sql) or die('Query failed103: ' . mysql_error());

//虚拟发货直接改为已收货   
if($express_id==0){
	
	$sql2="update weixin_commonshop_orders set sendstatus = 2,confirm_receivetime=now(),expressnum='".$expressnum."',send_remarks='".$remarks."' where batchcode='".$batchcode."'";
